...
**Файл:** `Docker/master/barkfluff/docker-compose.yml` (beacon, settings, files, identity, messages, notification, users, fast-auth, updates, onliner)
**Проблема:** Все микросервисы публикуют свои plaintext-HTTP/2 (gRPC) порты 7000–7015 на всех интерфейсах. При этом nginx на хосте проксирует на них через loopback (`docker/nginx/barkfluff.single-server.conf:2,6,10` — upstream'ы `127.0.0.1:*`). Settings в актуальном compose наружу не публикуется.
**Почему это проблема:** Внешний клиент может обойти nginx (TLS-терминацию, rate-limiting, маршрутизацию) и обращаться к сервисам напрямую по нешифрованному каналу. Межсервисные токены и трафик уязвимы. Docker-публикация обходит ufw (см. S2).
...
**Файл:** `Docker/{dev,nightly,master}/barkfluff/.env` (MinIO, RabbitMQ, Postgres, Settings DB и Seq credentials)
...
### S7. Секреты должны быть разделены между сервисами — High
**Файл:** `Docker/{dev,nightly,master}/barkfluff/docker-compose.yml`; bootstrap-compose для Settings/Setup
**Проблема:** Передача полного `.env` в контейнеры раскрывает токены Telegram, SSH-пароли, Firebase и креды инфраструктуры. Settings получает только `SETTINGS_*`, а Setup — отдельный Docker secret; остальные сервисы должны получать минимальный набор переменных.
**Почему это проблема:** Нарушение принципа наименьших привилегий: компрометация одного сервиса (или просто `docker inspect` / чтение `/proc/1/environ` любым, у кого есть доступ к docker) раскрывает секреты всей платформы и **других физических серверов** (SSH root).
**Рекомендация:** Не использовать `env_file: .env` для Settings/Setup и разделять секреты по Docker secrets или файлам с минимальными правами.
...
**Почему это проблема:** `restart: always` перезапускает только упавший процесс — зависший контейнер (deadlock, отвал gRPC) будет числиться `Up` бесконечно. `depends_on` без `condition: service_healthy` означает, что сервисы стартуют до готовности Settings/Postgres и полагаются на внутренние ретраи. Это удлиняет холодный старт и порождает шум ошибок в Seq.
