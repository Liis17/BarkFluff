...
### S17. Любой сервисный JWT даёт примитивы захвата произвольного пользовательского аккаунта — High
**Файлы:** общая политика класса — `Backend/BarkFluff.Identity/Host/IdentityServerApiService.cs:19-20`; создание сессии — `:81-95` и `Features/CreateSessionForUserServer/CreateSessionForUserServerCommandHandler.cs:31-74,136-143`; принудительная смена пароля — `Host/IdentityServerApiService.cs:98-106` и `Features/ForceSetPasswordServer/ForceSetPasswordServerCommandHandler.cs:34-62`.
**Проблема:** Весь `IdentityServerApiService` защищён только общей политикой `TokenType.Service`. `CreateSessionForUserServer` принимает `UserId` и данные устройства из запроса, создаёт для указанного пользователя access/refresh-токены и возвращает их вызывающему. `ForceSetPasswordServer` аналогично принимает произвольные `UserId` и новый пароль. Проверки конкретного сервиса-вызывателя, audience/scope или связи caller-а с целевым пользователем нет.
**Почему это проблема:** Компрометация JWT любого внутреннего сервиса превращается в полный захват любого не-bot аккаунта без пароля и 2FA. Тот же класс также позволяет общему service-токену перечислять/удалять сессии и отключать OTP. Это отдельная проблема авторизации RPC, а не срок жизни service-токена из S8.
**Рекомендация:** Разделить полномочия по identity конкретного сервиса: разрешить создание сессии только FastAuth с отдельным audience/scope, принудительную смену пароля — только AdminPanel с отдельным scope. Проверять service identity на каждом привилегированном RPC; не считать любой `TokenType.Service` достаточным.

### S18. Refresh-токены хранятся в БД в открытом виде — Medium
**Файлы:** `Backend/BarkFluff.Identity/Domain/RefreshToken.cs:5-18`; `Persistence/Services/RefreshTokensStorage.cs:11-33`.
**Проблема:** Bearer-значение refresh-токена записывается в колонку `Value` без хеширования и затем ищется прямым сравнением `x.Value == refreshToken`.
**Почему это проблема:** Read-only доступ к БД, дампу или резервной копии сразу даёт готовые токены всех активных сессий. Их можно обменять на access-токены без взлома пароля; ущерб особенно велик из-за срока около 27 лет (S7). S10 описывает TOTP/OTP/reset-секреты, но не bearer refresh-токены.
**Рекомендация:** Генерировать refresh-токен из достаточной энтропии, хранить только SHA-256 digest и искать по digest. Ротировать токен при каждом использовании и одновременно сократить срок жизни согласно S7.
...
**Файл:** `docker/nginx/identity.conf:15-24`
...
- **Хардкод секретов в сервисе:** `JwtSettings.SecretKey` берётся из централизованного Settings-сервиса (`Program.cs:41`), захардкоженного ключа в коде/`appsettings*.json` нет. (Замечание уровня Low/архитектура: один симметричный HS256-ключ общий для всех сервисов и его длина нигде не валидируется — вне области Identity, но стоит учитывать вместе с аудитом XAuth.)
