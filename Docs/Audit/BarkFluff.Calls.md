...
**SQL-инъекции (доп. проверка 2026-07-22):** не найдены. `CallsService` обращается к `CallsContext` только через EF Core LINQ (`Where`/`FirstOrDefaultAsync`), raw-SQL в сервисе отсутствует.
