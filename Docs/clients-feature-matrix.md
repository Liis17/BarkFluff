...
| Задача | Комментарий |
|---|---|
| Markdown в сообщениях | В macOS/iOS добавлены общий `BFCore` parser/AST и `BFMarkdown` SwiftUI renderer; для WPF V2 задача остаётся отдельной |
...
4. **Markdown.** Для macOS/iOS выбран и реализован паритет с Android/Web; для WPF V2 решение ещё не принято.
