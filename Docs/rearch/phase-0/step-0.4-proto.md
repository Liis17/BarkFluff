...
- И-1 закрыт решением в [../02-trust-and-certs.md](../02-trust-and-certs.md): хешируются **полученные wire-байты** запроса (без пере-сериализации), конверт `SignedRequest { bytes payload }` отвергнут — тело запросов не меняется, контракт стабилен. В Фазе 0 ничего дополнительно делать не нужно.
