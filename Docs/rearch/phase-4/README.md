...
- **Privacy применяет Onliner, а не Federation** (инвариант №27 — фильтрует владелец данных): у Onliner уже есть `OnlineVisibilityFilter` и клиент `Users.GetUserPrivacy`. Federation не заводит собственный кеш privacy и не дублирует правило «`OnlineVisibility != All` ⇒ скрыт»; он спрашивает статус наблюдаемого пользователя у Onliner (`GetLocalPresence`, 4.2), а тот отдаёт уже отфильтрованный (`PRESENCE_STATUS_UNKNOWN` для скрытых). Стоимость вызова гасится coalescing'ом: статусы уходят в S2S-стрим не чаще раза в несколько секунд на пользователя.
