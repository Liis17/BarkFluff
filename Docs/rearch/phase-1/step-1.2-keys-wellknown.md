...
**HTTP-endpoint**: gRPC-порт Kestrel сконфигурирован под HTTP/2 (h2c) — HTTP/1-GET на нём не живёт. Поднять **второй listener** HTTP/1 на порту из нового конфиг-ключа `Federation:WellKnownPort` (дефолт 7031 в коде; проверь по `ConfigurationDefaultsPopulator`, что 7031 никем не занят) и `app.MapGet("/.well-known/barkfluff", ...)`. Как именно `SetRunningAddress` настраивает Kestrel — прочитай в `Backend/BarkFluff.GrpcServer/` и добавь listener, не сломав gRPC. Nginx-проксирование apex-пути на этот порт — этап 1.6.
