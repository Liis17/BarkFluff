...
2. **Стенд** `Backend/dev-federation-testbed/`: `docker-compose.node2.yml` — мини-стек второй ноды (postgres2 + settings2 + federation2, свои volume/имена/порты, та же docker-сеть или своя с мостом), `seed-peers.sql` — вставка KnownServers/KnownServerKeys каждой ноды в БД другой (endpoint'ы plaintext `http://federation:7030` — TLS и пиннинг проверяются в 1.6 через nginx), `README.md` стенда: как поднять, как задать `Federation:ServerName` обеим нодам через Settings, как прогнать Ping.
