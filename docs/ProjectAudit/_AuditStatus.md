...
| OPT-03 — Transient вместо Scoped | ✅ исправлено | `Program.cs:68` → `AddScoped<ConfigurationStorage>()`. |
| OPT-04 — `Any()` вместо `Count == 0` | ✅ исправлено | `ConfigurationDefaultsPopulator.cs:125` → `if (emptyConfigs.Count == 0)`. |
...
- `GenerateServiceToken` (строка 372) создаёт JWT с `expires: DateTime.UtcNow.AddYears(10)` — служебный токен живёт 10 лет без ротации. При утечке — компрометация на годы.
...
| ID | Статус | Комментарий |
|----|--------|-------------|
| OPT-05 — `GrpcChannel` не Dispose | ✅ исправлено | `WebApplicationBuilderExtensions.cs:69` → `using var channel = ...`. |
| BUG-01 — опечатка `baseExcetion` | ✅ исправлено | `ServerExceptionInterceptor.cs:73` → `baseException`. |
| QA-03 — `RequestContext` мутабельный | ✅ исправлено | Все поля переведены на `{ get; init; }`. |
| QA-04 — опечатка `OperationSystem` | ❌ актуально | `Tracker/RequestContext.cs:5` всё ещё `OperationSystem`. Переименование требует правки `RequestContextInterceptor` и всех потребителей. |
...
Файл в самом аудите помечен `Статус: ✅ Исправлено` — сверка подтверждает.

| ID | Статус | Комментарий |
|----|--------|-------------|
| PERF-03 — повторные `string.Replace` | ✅ исправлено | `HtmlEmailTemplateParser` использует `[GeneratedRegex]` + `Regex.Replace` (single-pass, source-generated). |
| BUG-04 — лишний `AddMediatR` | ✅ исправлено | В `Program.cs` MediatR больше не регистрируется. |
| MISC-02 — email в логах как PII | ✅ исправлено | `EmailQueueConsumer` использует `EmailMasker.Mask(...)`. |
...
| ID | Статус | Комментарий |
|----|--------|-------------|
| OPT-01 — 7 последовательных await | ✅ исправлено | `GetServerInfoCommandHandler.Handle` запускает все 7 задач и ждёт через `Task.WhenAll`. |
| OPT-03 — лишний `.ToList()` в `ParseService` | ✅ исправлено | Сигнатура `ParseService(ServiceId, IEnumerable<ConfigurationItem>)`, без `.ToList()`. |
| BUG-01 — двойная регистрация MediatR | ✅ исправлено | В `Program.cs` единственный `AddMediatR` (строка 45). |
| MISC-05 — non-nullable без инициализации | ✅ исправлено | `ServerColorSettings` и `ServerPropsSettings` имеют `= string.Empty`. |
...
| ID | Статус | Комментарий |
|----|--------|-------------|
| PERF-01 — последовательная отправка FCM | ✅ исправлено | `PushNotificationConsumer` вызывает `SendNotificationBatchAsync` (FirebaseService.cs:72). |
...
| BUG-04 — бесконечные retry без DLQ | ✅ исправлено | Принят вариант «логировать без throw» — `catch (Exception)` без re-throw (строки 119-128). |
...
| ID | Статус | Комментарий |
|----|--------|-------------|
| SEC-03 — нет валидации длины/формата строк | ✅ исправлено | В `RegisterServerCommandHandler` теперь есть `MaxNameLength=64`, `MaxDescriptionLength=512`, `MaxBeaconHostLength=2048`, валидация порта (1-65535), `HexColorRegex` через `[GeneratedRegex]`, `Uri.CheckHostName` для BeaconHost. |
| OPT-02 — `servers.Count()` LINQ | ✅ исправлено | `ListServersQueryHandler.cs:31` → `servers.Count` (свойство). |
| OPT-03 — `_lastRegistrationTimes` растёт вечно | ✅ исправлено | `CleanupExpiredThrottleEntries` вызывается при каждом `RegisterServer` (`ServersStorage.cs:51`). |
| OPT-04 — псевдо-async `GetServers` | ✅ исправлено | Сигнатура `public List<ServerInfo> GetServers()` — синхронно. |
| BUG-01 — испорченная кодировка строки | ✅ исправлено | Корректная кириллица в `ServersStorage.cs:39`. |
| MISC-01 — `IMemoryCache` лишний | ✅ исправлено | Прямые поля `ConcurrentDictionary` без `IMemoryCache`. |
...
| ID | Статус | Комментарий |
|----|--------|-------------|
| SEC-01 — fail-open в `OnlineVisibilityFilter` | ✅ исправлено | `OnlineVisibilityFilter.cs:69` → `return false` + лог "fail-closed". |
| PERF-03 — `GetStreamsTrackingUser` O(N×M) | ✅ исправлено | Добавлен `_reverseIndex: TrackedUserId → ConnectionId → Stream`. Lookup O(1). |
| PERF-04 — `active_subscriptions` только инкрементируется | ✅ исправлено | Метрика переведена на gauge через `MetricsSnapshotService` (см. комментарий `OnlinerApiService.cs:65-66`). |
| PERF-05 — `GetAllStatuses` полная копия | ✅ исправлено | `_statuses.Values.ToList()` без `.Select(new ...)`. `UserOnlineStatus` — record. |
| BUG-02 — race condition в `statusChanged` | ✅ исправлено | CAS-loop через `TryUpdate`/`TryAdd` с возвратом true/false (`UpdateStatus`, `SetOffline`). |
...
| ID | Статус | Комментарий |
|----|--------|-------------|
| SEC-01 — timing-атака на токен | ✅ исправлено | `TokenAuthMiddleware.cs:35` → `CryptographicOperations.FixedTimeEquals`. Токен предкодирован в `_uploadTokenBytes`. |
| SEC-04 — S3-креды пустые | ✅ исправлено | `S3StorageService.cs:45-48` → `throw InvalidOperationException` при отсутствии. |
| BUG-02 — `CacheWarmupService.StopAsync` не ждёт задачу | ✅ исправлено | Сохранена `_warmupTask` + `_cts.CancelAsync()` + `await _warmupTask.WaitAsync(...)`. |
| BUG-03 — `FileStream` без try/catch | ✅ исправлено | `ClientStorageController.cs:285-294` — `try { ... } catch { fs?.Dispose(); throw; }`. |
...
| BUG-06 — `TryParseRange` не понимает `bytes=N-` | ✅ исправлено | `S3StorageService.cs:125-145` — поддержка open-ended с `to = -1` и `ByteRange(from, long.MaxValue)`. |
...
| ID | Статус | Комментарий |
|----|--------|-------------|
| OPT-01 — `DownloadAsync` в `MemoryStream` | ✅ исправлено | `S3Uploader.cs` возвращает `S3ObjectStream` — обёртку над `GetObjectResponse` без буферизации. Аккуратно проксирует `Dispose` на response. |
| OPT-03 — повторное декодирование изображения | ✅ исправлено | `ProcessImageAllInOneAsync` — один `Image.LoadAsync` для resize оригинала + превью + размеры (`UploadFileCommandHandler.cs:259`). `Image.IdentifyAsync` (строка 190) читает только заголовок — допустимо. |
| OPT-04 — N+1 INSERT в `TempFiles` | ✅ исправлено | `CreateTempFilesBatchAsync` в `TempFilesStorage.cs:38-53` используется в `GetTempDownloadUrlCommandHandler.cs:61`. |
| BUG-04 — нет очистки `TempFile` | ✅ исправлено | `DeleteExpiredAsync` + `TempFileCleanupService` зарегистрирован в `Program.cs:61` как `HostedService`. |
| MISC-01 — SecretKey в кэше клиентов | ✅ исправлено | `S3BucketRegistry.cs:98-99` — `Convert.ToHexString(SHA256.HashData(...))` вместо raw. |
| MISC-02 — `ImageCompressor` как Scoped | ✅ исправлено | `Program.cs:60` → `AddSingleton<ImageCompressor>()`. |
...
| ID | Статус | Комментарий |
|----|--------|-------------|
| SEC-03 — нет лимита длины текста | ✅ исправлено | `MaxTextLength = 4096` в handler + `HasMaxLength(4096)` в `MessageConfiguration`. |
| SEC-04 — нет лимита вложений | ✅ исправлено | `MaxAttachmentsPerMessage = 10` + `TooManyAttachmentsException`. |
| OPT-02 — `CheckAccessToChat` тянет всю сущность | ✅ исправлено | `ChatsStorage.cs:80-84` → `_context.ChatMembers.AnyAsync(...)`. |
| OPT-08 — `GetUserAllMessages` без стриминга | ✅ исправлено | `AsAsyncEnumerable()` + `await foreach` в `GetUserAllMessagesQueryHandler.cs:40-46`. |
...
| BUG-02 — `KickUser` ищет по `Id` вместо `UserId` | ✅ исправлено | `KickUserCommandHandler.cs:64` → `x.UserId == request.UserId`. |
| BUG-04 — `KickUser` сначала очередь, потом БД | ✅ исправлено | `AddMessage` на 115 строке перед `SendMessage` на 119. |
| MISC-01 — `ChatCache` глотает исключения | ✅ исправлено | Все 4 `catch` блока теперь логируют через `_logger.LogWarning(ex, ...)`. |
| MISC-02 — sync `FirstOrDefault` в async | ✅ исправлено | `MessagesStorage.cs:30,79,151` — все `FirstOrDefaultAsync`. |
| MISC-04 — пробелы в системном сообщении | ✅ исправлено | `"Создан групповой чат \"{request.Title}\""` — пробелы убраны. |
...
| ID | Статус | Комментарий |
|----|--------|-------------|
| SEC-01 — SHA-256 без соли | ✅ исправлено | `PasswordHasher` использует `BCrypt.Net.BCrypt` с workFactor=12. Legacy SHA-256 поддерживается для миграции старых паролей (через `FixedTimeEquals`). |
| SEC-04 — `System.Random` для refresh token | ✅ исправлено | `RandomNumberGenerator.GetBytes(32)` + URL-safe Base64. |
| SEC-07 — утечка существования пользователя | ✅ исправлено | `ResetPasswordCommandHandler.cs:90-101` — `Task.Delay(Random.Shared.Next(100, 300))` + фиктивный `ResetId = Guid.NewGuid()`. |
| SEC-08 — нет `ExpiresAt` у `ResetPassword` | ✅ исправлено | Поле `ExpiresAt` в `ResetPassword.cs:13`. `AddMinutes(15)` для authenticator, `AddMinutes(5)` для email. Проверка в `ConfirmResetPasswordCommandHandler.cs:94-104`. |
| SEC-09 — нет удаления использованного кода | ✅ исправлено | `ConfirmationCodesStorage.DeleteCode` + вызов в `ConfirmAccountCommandHandler.cs:101`. |
| PERF-04 — нет индекса на `RefreshTokens.Value` | ✅ исправлено | `IdentityContext.cs:26` → `HasIndex(x => x.Value)`. Миграция `20260507005955_SecurityHardening`. |
| PERF-06 — lock + `static Random` в `CodeGenerator` | ✅ исправлено | `RandomNumberGenerator.GetInt32(0, 10)` — без lock, криптостойко. |
| BUG-01 — `DeviceId ?? DeviceName` | ✅ исправлено | `ConfirmResetPasswordCommandHandler.cs:154-156` — `Guid.NewGuid().ToString()` при отсутствии. |
| BUG-03 — не проверяется тип кода | ✅ исправлено | `ConfirmAccountCommandHandler.cs:55` — `if (code.Type != ConfirmationCodeType.Registration)`. |
| CODE-01 — дублирование `locationInfo` | ✅ исправлено | Extension `GetLocationString` используется во всех 12+ handler'ах. |
...
| MISC-03 — record vs class | ✅ исправлено | Все Notification-классы — `record`. |
...
| ID | Статус | Комментарий |
|----|--------|-------------|
| SEC-01 — `EnsureHttpPrefix` по умолчанию `http://` | ✅ исправлено | `WebApi.cs:156-161` — комментарий "По умолчанию — https://". |
| SEC-02 — двойная инъекция Bearer | возможно ✅ | Не найдено вручную добавляемых `Authorization`/`Bearer` в managers. |
...
## Итог по четырём группам (28 файлов — все аудиты)

Финальная сводка. Числа — приблизительные (некоторые пункты в четвёртой группе не разбирал детально из-за объёма, отмечал общий тренд).

| Проект | Исправлено | Актуально | Частично | Не проверял | Новых |
|--------|:----------:|:---------:|:--------:|:-----------:|:-----:|
| Configuration         | 2  | 1  | 1 | 0  | 2 |
| GrpcServer            | 3  | 1  | 0 | 0  | 2 |
| Notification          | 3  | 0  | 0 | 0  | 1 |
| Beacon                | 4  | 0  | 0 | 0  | 2 |
| CloudMessaging        | 2  | 0  | 1 | 0  | 3 |
| Navigator             | 6  | 0  | 0 | 0  | 2 |
| Onliner               | 5  | 1  | 0 | 0  | 1 |
| ClientStorage         | 5  | 0  | 1 | 0  | 1 |
| Files                 | 6  | 0  | 1 | 0  | 1 |
| Messages              | 9  | 0  | 0 | 0  | 2 |
| Identity              | 10 | 0  | 0 | 0  | 3 |
| SecurityUtilities     | 0  | 9  | 2 | 0  | 0 |
| Shared.Exceptions     | 0  | 14 | 0 | 0  | 0 |
| Shared.Queue          | 0  | 17 | 0 | 0  | 0 |
| Developers            | 0  | 16 | 0 | 0  | 0 |
| Shared.Auth           | 0  | 14 | 0 | 0  | 1 |
| Proto                 | 0  | 19 | 1 | 0  | 1 |
| Barkfluff (macOS)     | 1  | 3  | 0 | 14 | 0 |
| WebServer             | 0  | 5  | 0 | ~23| 0 |
| FastAuth              | 0  | 5  | 0 | ~11| 0 |
| AdminPanel            | 1  | 2  | 1 | ~17| 0 |
| Client.Android        | 0  | 2  | 0 | ~17| 0 |
| Users                 | 0  | 4  | 0 | ~15| 0 |
| Updates               | 1  | 2  | 1 | ~12| 0 |
| WebApi.Core           | 1  | 0  | 0 | ~16| 0 |
| Shared.Identity       | ?  | 4  | 0 | ~17| 0 |
| Client.WPF            | 0  | 1  | 0 | ~22| 0 |
| Web                   | 0  | 2  | 0 | ~17| 0 |
...
Третья группа shared-библиотек — это **фундамент кросс-сервисного взаимодействия** (контракты, исключения, очереди, auth). То, что её не правили, означает:

- При каждом фиксе в backend-сервисе из первых двух групп **базовые контракты остаются проблемными** — например, `Messages.SEC-03` фиксил длину текста, но `Shared.Queue.SEC-01` всё ещё пропускает её через RabbitMQ в FCM.
- Изменения в `Shared.Exceptions` затронут все микросервисы (общий пакет), поэтому сложнее, но и пользы больше.

Если делать ещё одну итерацию правок — начать стоит с **`Shared.Auth` + `Shared.Exceptions`** (это база безопасности + единообразия ошибок) и **`Shared.Queue` BUG-02 + SEC-01** (надёжность очередей).
