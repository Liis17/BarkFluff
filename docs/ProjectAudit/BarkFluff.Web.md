...
### SEC-03 — Пароль передаётся в protobuf без клиентской валидации сложности
...
### PERF-04 — `markVisibleMessagesAsRead` использует `querySelectorAll('.msg-bubble')` на каждый scroll-тик
...
### BUG-02 — `handleMessageRead` уменьшает `countUnread` на 1 при каждом событии для закрытых чатов
...
### BUG-05 — `attach.js` не проверяет `currentChatId` перед отправкой файлов
...
### BUG-06 — `loadChats` неверно вычисляет `chatListOffset` после сортировки
