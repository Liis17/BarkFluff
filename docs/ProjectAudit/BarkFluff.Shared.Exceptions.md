...
> ⚠️ **Статус (2026-05-18):** Частично исправлена. Для BaseGrpcException теперь используется ex.ErrorMessage (контролируемая строка). Утечка системного ex.Message остаётся только в ветке catch (Exception) на строке 79.
...
> ⚠️ **Статус (2026-05-18):** Частично исправлена. Для BaseGrpcException используется StatusCode.FailedPrecondition (осмысленно). Для непредвиденных исключений на строке 79 остаётся StatusCode.Unknown.
...
| NEW-BUG-01 | 🐛 Баг | 🟢 Низкая | `ProfilePictureHasNotValidType` не оканчивается на "Exception" |
| NEW-BUG-02 | 🐛 Баг | 🟢 Низкая | Опечатка `зарегристирован` в ErrorMessage `EmailExistException` |
| NEW-SEC-01 | 🔒 Безопасность | 🟡 Средняя | Все ErrorMessage хардкодены на русском без i18n |
| NEW-MISC-01 | 🧹 Качество | 🟡 Средняя | Синтетические (non-random) GUID в ErrorCode минимум 11 классов |
| NEW-ARCH-01 | 🏗️ Архитектура | 🟡 Средняя | `ServerExceptionInterceptor` не обрабатывает streaming-вызовы |
