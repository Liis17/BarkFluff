...
Критичных и высоких проблем не найдено. Основные риски — среднего уровня: отсутствие rate limiting на анонимных проверках существования (энумерация) и на выдаче prekey-бандлов (исчерпание one-time prekeys), PII (email) в логах без маскирования, а также отсутствие `AsNoTracking` и пагинации на ряде read-only/broadcast-запросов.
...
**Файл:** `Backend/nginx/users.conf:15`–`:24`
