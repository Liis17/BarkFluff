...
> Дата: 2026-06-12. Область: docker/nginx/*.conf, BarkFluff.Web/nginx, Frontend/Developers/nginx.conf.
...
**Файл:** `docker/nginx/admin-panel.conf:15-24`
...
**Файл:** `docker/nginx/identity.conf:15-24`, `docker/nginx/fast-auth.conf:15-24`, `docker/nginx/admin-panel.conf:15-24`
...
**Файл:** `Frontend/Developers/nginx.conf:33`, `docker/backend/docker-compose-dev-backend.yml:169-170`
...
**Файл:** `docker/nginx/01-ssl-params.conf:1-11` (общий сниппет — естественное место), а также все server-блоки 443
...
**Файл:** `docker/nginx/admin-panel.conf:8-25`, `docker/nginx/web.conf:8-40`, `docker/nginx/developers.conf:8-25`, `Frontend/Developers/nginx.conf:1-45`, `docker/nginx/barkfluff.single-server.conf:52-101` (location `/web/`)
...
**Файл:** `docker/nginx/01-ssl-params.conf:8,11`, `docker/nginx/barkfluff.single-server.conf:28-29,62-63`, `Frontend/Developers/nginx.conf:11-12`
...
**Файл:** `docker/nginx/barkfluff.single-server.conf:18-29,52-63`, `Frontend/Developers/nginx.conf:1-12`
...
**Файл:** `docker/msk/docker-compose-msk.yml:5,10-13`
...
**Файл:** `docker/nginx/barkfluff.single-server.conf:18-21,52-55`
...
**Файл:** `docker/nginx/updates.conf:22-23`, `docker/nginx/onliner.conf:22-23`
...
### P2. docker/nginx/web.conf устарел: буферизация и 300s ломают gRPC-Web стримы; дубль с BarkFluff.Web/nginx/web.conf — High
**Файл:** `docker/nginx/web.conf:30-39` (проблемный location), ср. `Backend/BarkFluff.Web/nginx/web.conf:52-72` (исправленный вариант)
**Проблема:** В репозитории два конфига для `web.barkfluff.com`. Старый (`docker/nginx/web.conf`, последнее изменение 2026-04-15, коммит 452d0bb1) проксирует весь трафик — включая gRPC-Web server-streaming Updates/Onliner — через `location /` без `proxy_buffering off`, без `proxy_http_version 1.1` (апстрим получает HTTP/1.0 без keepalive) и с `proxy_read_timeout 300s`. Новый (`Backend/BarkFluff.Web/nginx/web.conf`, 2026-06-06, коммит 067188bf) всё это исправляет: отдельный location для стримов, buffering off, 24h, keepalive, security-заголовки. Также в старом `client_max_body_size 512m` задан только для `/api/files/upload/` (строка 23) — gRPC-Web вызовы с телом >1 МБ через `location /` получат 413.
...
**Рекомендация:** Удалить `docker/nginx/web.conf` или заменить его содержимым из `Backend/BarkFluff.Web/nginx/web.conf` (адаптировав upstream `127.0.0.1:7016` → `web:7016`, если nginx в Docker-сети). Один домен — один конфиг-источник истины.

### P3. client_max_body_size не задан для gRPC-локаций (дефолт 1 МБ) — Medium
**Файл:** `docker/nginx/files.conf:16-25` (главное), а также gRPC-локации `identity.conf:15`, `users.conf:15`, `messages.conf:15`, `beacon.conf:15`, `fast-auth.conf:15`, `updates.conf:15`, `onliner.conf:15`
...
**Файл:** `docker/nginx/developers.conf:8-25` и `Frontend/Developers/nginx.conf:1-45`
...
**Файл:** `docker/msk/docker-compose-msk.yml:8-13`; `docker/backend/docker-compose-dev-backend.yml`, `Backend/docker-compose-master.yml` (сервиса nginx нет)
...
**Файл:** все per-service конфиги, например `docker/nginx/identity.conf:16-17` (`set $grpc_backend ...; grpc_pass $grpc_backend;`)
...
**Файл:** `docker/nginx/barkfluff.single-server.conf:65,87-100`; синтаксис `listen 443 ssl http2` — все per-service конфиги (например `identity.conf:9`)
