...
### S2. Итоговые токены выдаются по знанию ID, закодированного в самом QR — Medium
...
### S3. IP-адрес в карточке подтверждения берётся из спуфабельных источников — Medium
...
### S4. Секретный session id пишется в логи в открытом виде — Medium
...
### S5. Identity-сессия создаётся до финальной атомарной проверки — осиротевшие валидные сессии — Low
...
### S6. gRPC reflection включён безусловно (в т.ч. в проде) — Low
