...
Распределение по категориям: Безопасность — 10, Производительность — 6, Docker/nginx — 3.
...
### S10. Любой сервисный JWT позволяет отправлять сообщения от имени произвольного пользователя — High
**Файлы:** общая политика класса и серверный RPC — `Backend/BarkFluff.Messages/Host/MessagesServerApiService.cs:20-21,101-141`; выбор отправителя и проверка членства — `Features/SendMessage/SendMessageCommandHandler.cs:63-67,120-190`; сохранение сообщения — `:319-330`.
**Проблема:** `SendMessageServer` защищён только общей политикой `TokenType.Service`, принимает `SenderUserId` и `AllowChatCreation` от вызывающего и прямо переносит их в команду. Хендлер проверяет членство в чате именно для подставленного `SenderUserId`, а затем сохраняет его как `Message.SenderId`. Проверки, что caller является сервисом Bots и что указанный пользователь действительно принадлежит этому боту/caller-у, нет.
**Почему это проблема:** После утечки токена любого внутреннего сервиса атакующий может публиковать сообщения, визуально и логически принадлежащие произвольному пользователю или боту; при `AllowChatCreation` — также инициировать личные чаты от его имени. S4/S5 описывают чтение membership/chat info сервисными токенами, но не эту write-операцию с impersonation.
**Рекомендация:** Выдать Bots отдельный audience/scope для серверной отправки и проверять его в `SendMessageServer`. Связать caller с разрешённым bot userId на стороне Messages; не принимать произвольный `SenderUserId` и `AllowChatCreation` только на основании общей service-политики.
