...
### S4. Сервис доступен напрямую с хоста в master без auth и без TLS — Critical
...
### S5. Управляющий аудит-след (`EditedBy`/`EditedFrom`) задаётся клиентом — High
...
### S6. Секреты хранятся в БД открытым текстом — High
...
### S7. Хардкод дефолтных кредов в Populator (minio/guest) — Medium
...
### S8. Сервисные токены живут 10 лет и неотзываемы — Medium
...
### S9. Строки подключения с паролем БД могут попадать в логи (Seq) — Medium
...
### D1. Публикация порта Configuration на хост в master — см. S4 (Critical)
