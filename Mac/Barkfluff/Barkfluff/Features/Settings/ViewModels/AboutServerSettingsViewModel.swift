...
//  Barkfluff
//
//  Загружает список эндпоинтов микросервисов из ConnectionManager
//  и обрабатывает пинг текущего сервера через ServerDiscoveryService.
//
...
final class AboutServerSettingsViewModel {
    /// Эндпоинты микросервисов, которые знает текущая сессия.
    var services: [ServiceEntry] = []
    /// Адрес Beacon (host, port) сохранённого сервера, если есть.
    var beaconHost: String?
    var beaconPort: Int?

    /// Состояние последнего пинга.
