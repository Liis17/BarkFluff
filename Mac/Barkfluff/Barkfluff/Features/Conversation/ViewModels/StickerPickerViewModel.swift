...
final class StickerPickerViewModel {
    ...
    var selectedTab: Tab?
    /// Стикеры по id пака (заполняются по мере выбора).
    private(set) var stickersByPack: [String: [Sticker]] = [:]
    /// Восстановленные «недавние» стикеры (по id из RecentStickersStore).
    ...
    /// Колбэк, который выставляет владелец (StickerPickerView) перед .task.
    ...
    /// Первичная загрузка. Идемпотентна.
    func loadIfNeeded() async {
        ...
        do {
            ...
            // Подтянем содержимое всех паков для обложек и быстрого показа.
            for pack in loaded {
            ...
            // Авто-выбор начального таба: «Недавние» если есть, иначе первый пак.
            if selectedTab == nil {
    ...
    /// Сменить активную вкладку. Если выбран пак, у которого ещё нет стикеров — подтянем.
    ...
    /// Тап по стикеру: запоминаем в недавних, делегируем владельцу.
    func didTap(_ sticker: Sticker) {
        recentStore.add(sticker.id)
        onStickerSelected?(sticker)
    }

    /// Видимые стикеры активного таба с учётом фильтра поиска по emoji.
    ...
    /// Стикер-обложка пака: ищем в его содержимом по `coverStickerID`.
