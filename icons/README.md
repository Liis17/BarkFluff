...
`admin/` содержит иконки навигации AdminPanel, управления контейнерами и
состояний массовых действий: запуск, остановка, перезапуск, обновление,
удаление, синхронизация и результат операции. Они используются актуальным
UI в `Backend/Barkfluff.AdminPanel/Pages/v2/` и доступны для других страниц
через общий helper `assets/icons.js` (`bfIcon('admin/restart', 'size-20')`).
