...
Дефолтные (плейсхолдер) иконки типов устройств — для списков сессий/устройств пользователя (`Users`/`Identity`), когда нет специфичной иконки конкретной модели. Префикс `default-` явно маркирует их как заглушку, а не как иконку бренда. Тот же язык M3: `viewBox 0 0 24 24`, `stroke-width:2`, `round` caps/joins, `currentColor`, точечная заливка только для мелких деталей (камера, dynamic island).
