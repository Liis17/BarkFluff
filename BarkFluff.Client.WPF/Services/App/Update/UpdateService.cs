...
namespace BarkFluff.Client.WPF.Services.App.Update
{
    public class UpdateService
    {
        ...
        public UpdateService(string currentVersion, string currentType)
        {
            ...
            _timer = new System.Timers.Timer(2000); // 5 минут = 300000 мс
        ...
        private async Task CheckForUpdatesAsync()
        {
            try
            {
                ...
                {
                    string downloadUrl = latestSuitable.Assets?.Find(a => a.Name == "wpf-build.zip")?.BrowserDownloadUrl;
                    if (!string.IsNullOrEmpty(downloadUrl))
                    {
                        // Вызов события или метода для обновления, например: OnUpdateAvailable(downloadUrl);
                        WPF.App.ErideMessage.AddMessage($"Обновление доступно: {latestSuitable.TagName}, скачать {downloadUrl}", new Erida.MessageType { Type = Erida.MessageType.MessageTypeEnum.Info });
                    }
                }
            }
            catch (Exception ex)
        ...
        private void OnUpdateAvailable(string url) => UpdateAvailable?.Invoke(url);
    }
}
