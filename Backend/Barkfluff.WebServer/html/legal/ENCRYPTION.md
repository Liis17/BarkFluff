...
- `Identity.RefreshTokens.Value` — значения токенов обновления
...
**Назначение:** Redis используется в сервисе **Messages** для **кеширования метаданных чатов** (имена и изображения чатов) — это технический кеш без персональных сообщений.

**Что хранится в Redis:**
- Имена чатов (`chat_name_{chatId}_{userId}`) — отображаемые названия для списка чатов
- Аватары чатов (`chat_image_{chatId}_{userId}`) — URL превью изображений чатов

**Что НЕ хранится в Redis:**
- Содержимое сообщений (хранится только в PostgreSQL)
- Онлайн-статусы пользователей (хранятся в in-memory кеше Onliner-сервиса + PostgreSQL)
...
**Хранение:**
- Токены хранятся в `Identity.RefreshTokens` в базе данных PostgreSQL
