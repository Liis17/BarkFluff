(() => {
    ...
    async function loadState() {
        try {
            ...
            $("#logout").hidden = false;
            $("#page-title").textContent = response.locked
                ? "Настройка завершена"
                : "Настройка сервера";
            render();
        } catch (error) {
            if (error.status === 401) {
                ...
                setMessage(
                    $("#login-error"),
                    error.message || "Settings пока недоступен.",
                    "error",
                );
            }
        }
    ...
    function fieldTemplate(field) {
        ...
        const configured = field.configured
            ? '<span class="configured">Сохранено</span>'
            : "";
    ...
    function controlTemplate(field, value, placeholder) {
        ...
        switch (field.inputType) {
            ...
            case "Color":
                return `<input ${common} type="text" value="${escapeAttr(value)}" maxlength="7">`;
            case "Integer":
                return `<input ${common} type="number" value="${escapeAttr(value)}">`;
            case "Secret":
                return `<input ${common} type="password" value="" autocomplete="new-password">`;
            case "Email":
                return `<input ${common} type="email" value="${escapeAttr(value)}">`;
            case "Boolean":
                return `<select ${common}><option value="false" ${value !== "true" ? "selected" : ""}>Выключено</option><option value="true" ${value === "true" ? "selected" : ""}>Включено</option></select>`;
            default:
                return `<input ${common} type="text" value="${escapeAttr(value)}">`;
        }
    }
    ...
    async function complete() {
        try {
            const response = await api("/api/setup/complete", {
                method: "POST",
                csrf: true,
                body: {},
            });
            ...
            setMessage(
                $("#action-message"),
                error.message || "Заполните все обязательные поля.",
                "error",
            );
        }
    }
