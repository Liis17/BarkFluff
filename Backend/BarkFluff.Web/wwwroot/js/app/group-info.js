...
(function () {
    ...
    function open() {
        ...
        loadMembers();
        deps.setMediaTabActive(
            "#groupOverlay .group-media-tab",
            deps.groupMediaPanels,
            "media",
        );
        ...
        BF.api
            ...
            .then(function (data) {
                ...
                members.forEach(function (member) {
                    ...
                    memberAvatar.textContent = (fullName ||
                        "?")[0].toUpperCase();
    ...
    function confirmRemoveMember(member, memberName) {
        if (
            !window.confirm(
                BF.i18n.t("group.removeMember.confirm", { name: memberName }),
            )
        )
            return;
        BF.api
    ...
    function rename() {
        ...
        var next = window.prompt(
            BF.i18n.t("newchat.groupTitle.placeholder"),
            title || "",
        );
        if (next == null) return;
        next = next.trim();
        ...
        BF.api
            ...
            .then(function (result) {
                var updatedTitle =
                    (result && result.chat && result.chat.title) || next;
    ...
    function addMember(userId, memberName) {
        BF.api
            ...
            .then(function () {
                ...
                loadMembers();
                deps.showToast(
                    BF.i18n.t("group.memberAdded", { name: memberName }),
                );
            })
            .catch(function () {
        ...
        BF.files
            ...
            .then(function (result) {
                ...
                if (picture) {
                    ...
                    deps.chatHeaderAvatar.innerHTML =
                        '<img src="' + deps.escapeHtml(picture) + '" alt="">';
                    var chatItem = deps.getChats().find(function (item) {
                        return item.id === chatId;
                    });
                    if (chatItem) {
            ...
            BF.api
                ...
                .then(function (data) {
                    ...
                    (data.users || []).forEach(function (user) {
                        ...
                        memberAvatar.className = "group-member-avatar";
                        var picture =
                            user.profilePicturePreview || user.profilePicture;
                        if (picture) {
                            var image = document.createElement("img");
                            image.src = picture;
                            image.alt = "";
    ...
    function init(options) {
        ...
        if (backgroundButton)
            backgroundButton.addEventListener("click", function () {
                var chat = deps.getCurrentChatInfo();
                deps.openChatBackgroundSelector(
