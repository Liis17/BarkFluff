...
public sealed partial class LoginViewModel : ObservableObject
{
    ...
    /// <summary>
    /// QR остаётся строкой base64: картинку собирает <c>Base64ToImageSourceConverter</c>,
    /// поэтому ViewModel не зависит от типов представления.
    /// </summary>
    [ObservableProperty]
    private string? _fastAuthQrCode;
    ...
    private async Task RunFastAuthAsync(CancellationTokenSource cancellation, int generation)
    {
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                FastAuthQrCode = ValidateBase64Png(session.QrCode.Base64Png);
    ...
    private static string? ValidateBase64Png(string base64Png) =>
        Convert.TryFromBase64String(base64Png, new byte[((base64Png.Length + 3) / 4) * 3], out var written) && written > 0
            ? base64Png
            : null;
}
