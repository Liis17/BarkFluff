...
| `Services/OldVersionsCleanupService.cs` | `IHostedService`. При старте удаляет записи и S3-объекты старше 7 дней, каждые 3 часа оставляет максимум 3 версии, ежедневно в 03:00 по Москве оставляет только последнюю версию по каждой паре `ClientType × ReleaseChannel`. |
