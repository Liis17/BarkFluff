...
| `OpenShellAsync` | Открывает интерактивный SSH-сеанс для браузерной консоли после WebSocket upgrade; пароль остаётся на стороне AdminPanel |
...
| GET (WebSocket) | `/api/remote/servers/{serverId}/console` | Сначала принимает WebSocket, затем открывает SSH-сеанс через PTY; ready/error-сигналы, auth cookie и same-origin `Origin` обязательны |
