...
| `OpenShellAsync` | Открывает интерактивный SSH-сеанс для браузерной консоли; пароль остаётся на стороне AdminPanel |
...
| GET (WebSocket) | `/api/remote/servers/{serverId}/console` | Интерактивный SSH-сеанс через PTY; требует auth cookie и same-origin `Origin` |
