...
### `utils.js` → `BF.utils`
Утилиты: форматирование дат/времени, `escapeHtml`, `parseJwt`, форматирование размера файла, прочие хелперы; модальные оверлеи (`openOverlay`/`closeOverlay` — focus-trap, `inert`) и клавиатура немодальных меню (`focusMenuItem`/`handleMenuKeydown`, см. [[Клиенты/Web#Контекстные меню (клавиатура и a11y)]]).
...
Список чатов в сайдбаре (`#chatList`, listbox): постраничная загрузка `load(reset)` (по 50, догрузка при скролле к низу), виртуализированный `render()` (в DOM только окно видимых строк, roving tabindex и клавиатурная навигация; см. [[Клиенты/Web#Список чатов (виртуализация)]]), тихий catch-up `refreshQuiet()` → `Promise<boolean>` (сравнение сигнатур первой страницы, DOM не трогается без изменений), подгрузка профилей собеседников пачками по 5, контекстное меню чата `#chatContextMenu` (добавить/убрать из папки, создать папку). Массив `chats` принадлежит `main.js` и передаётся через `getChats`/`setChats`; остальные deps: `getCurrentChatId`, `getMyUserId`, `getCachedUser`, `getUser`, `isUserOnline`, `collectOnlineUserIds`, `updateTitleBadge`, `openChat`, `botBadgeMarkup`. В `main.js` — алиасы `loadChats`/`renderChatList`/`refreshChatListQuiet`.

### `composer.js` → `BF.composer`
Поле ввода `#messageInput` и его состояние: Enter → отправка (`deps.onSubmit` = `sendMessage` в `main.js`), автовысота, отправка своего typing-статуса (`setTypingStatus` каждые 4 с, стоп через 5 с тишины), ответ/редактирование (`#replyPreviewBar`/`#editPreviewBar`), черновики (`restoreDraft`, сохранение на каждый ввод через `BF.drafts`), вставка файлов из буфера и drag-and-drop на `.chat-area` → `BF.attach.open` → `deps.onSendFiles` (`sendMessageWithFiles`). API: `getText`, `getReplyToId`, `getEdit`, `setReply`/`clearReply`, `setEdit`/`clearEdit`, `stopTyping`, `restoreDraft`, `restoreFromPending` (возврат отменённой отправки), `clearForSend`, `onMessageDeleted(id)` (удалено редактируемое сообщение — режим правки снимается, набранный текст остаётся; удалено сообщение, на которое готовится ответ, — ответ снимается; вызывается из `applyMessageDelete` в `main.js`), `openAttach`. Тест — `scripts/test-composer.js`. Пайплайн отправки (pending sends/uploads), `#sendBtn` и `#attachBtn`/`#fileInput` остаются в `main.js`; там же алиасы `setPendingReply`/`clearPendingEdit`/`stopTypingSend` и др.

### `feed.js` → `BF.feed`
Лента открытого чата (`#messagesInner` в `#messagesArea`): полный рендер `render()`, инкрементальная подгрузка старых (`prependMessages`) и новых (`loadNewerMessages`) страниц по 30 на скролле, скользящее окно `MAX_MESSAGES = 200` (`trimMessages`), `jumpToLiveTail`, группировка сообщений и разделители дат/непрочитанных, кнопка «вниз» с бейджем, `scrollToMessage` (прыжок ±30 с подсветкой), `loadMessagesPage` для обычных и приватных (расшифровка через `deps.decryptPrivateBatch`) чатов. Доступность: `role="feed"`/`article`, roving tabindex, клавиатурная навигация и live-регион новых входящих ([[Клиенты/Web#Лента как ARIA feed]]). Подробности окна — [[Клиенты/Web#Скользящее окно ленты]]. Deps: `getCurrentChatId/Type/Info`, `getMyUserId`, `getMessages`/`setMessages`, `getUser`, `showMediaOverlay`, `mergePendingUploads`, `onPendingCancel`/`onPendingRetry`, `decryptPrivateBatch`, `showToast`. API для остального кода: `replaceElement`/`removeElement` (замена узлов с переносом фокуса), `announceIncoming`.

### `message-menu.js` → `BF.messageMenu`
Контекстное меню сообщения `#msgContextMenu` (правый клик, долгое нажатие 500 мс на touch, Shift+F10 / клавиша ContextMenu на сфокусированном сообщении; `role="menu"` с навигацией стрелками) и свайп влево для ответа на мобильном. Меню показывает только применимые пункты (редактировать/удалить — свои несистемные, копировать изображение — единственная картинка), закрывается кликом снаружи, Escape, скроллом ленты и resize; в приватных чатах не открывается. Действия выполняются через deps: `setReply`/`setEdit` (композер), `forward(msg, id)` (модалка пересылки в `main.js`), `requestDelete`, `showToast`; закреп — через `BF.pinned`. API: `init`, `close` (в `main.js` — алиас `closeContextMenu`).
...
### `profile.js` → `BF.profile`
Панель профиля пользователя (`#profileOverlay`): `open(userId)` загружает `GetUser`, заполняет карточку (аватар, имя, `@username`, био, статус «в сети»/«был(а) в сети», регистрация, бейджи, id пользователя и чата), у бота показывает бейдж и скрывает статус и кнопки звонков, грузит вкладки медиа открытого чата через `BF.chatMedia` и открывает оверлей; ответ на устаревший `open` отбрасывается. Обработчики закрытия (крестик, подложка, «Написать сообщение»), кнопка фона чата (`BF.chatBackground.open`), копирование id в буфер (звук `success` и тост). Deps: `getCurrentChatId`, `getCurrentChatInfo`, `isUserOnline`, `getOnlineEntry`, `botBadgeMarkup`, `setCallButtonsVisible`, `showToast`. Клик по шапке чата (`onChatHeaderClick` в `main.js`) выбирает профиль собеседника или панель группы. Тест — `scripts/test-profile.js`.

### `forward.js` → `BF.forward`
Диалог пересылки (`#forwardOverlay`): `open(msg, fallbackId)` собирает id пересылаемых сообщений (`resolveSourceIds`: у пересланного сообщения пересылаются оригиналы по порядку `order`, иначе само сообщение), показывает список чатов с множественным выбором, счётчиком и блокировкой кнопки «Отправить» без выбора; отправка идёт последовательно в каждый выбранный чат с комментарием (пустой — `null`), ошибка одного чата не прерывает остальные, по завершении диалог закрывается и показывается тост `forward.done`. Закрытие: крестик, подложка, Escape. Deps: `getChats`, `showToast`. Меню сообщения вызывает `BF.forward.open` через свои deps. Тест — `scripts/test-forward.js`.

### `connection.js` → `BF.connection`
Состояние соединения и догон пропущенного: плашка `#connectionBanner` («Нет сети» / «Переподключаемся…», кнопка «Повторить» вызывает `BF.realtime.reconnect()` и блокируется до следующего статуса), чип `#connectionSynced` на 3 с после успешного догона (только при открытом чате). Первый `connection_status` не считается проблемой; после разрыва при `connected` запускается ровно один догон — `Promise.all([refreshChatList(), resyncCurrentChatTail()])`. Устаревший по эпохе статуса результат отбрасывается и проход повторяется; неудачный проход просит транспорт переподключиться. Событие `resync` дебаунсится на 1200 мс (нет открытого чата — только список; иначе только хвост чата), `tab_visible` запускает оба обновления. `resyncCurrentChatTail()` возвращает `Promise<boolean>`: `diffFetchedTail` сверяет свежий хвост с окном (новые, правки, прочтения, удаления только внутри диапазона выборки); хвостовые новые дописываются точечно (разделитель «Новые сообщения» на первом реально добавленном, `markSoon` для чужих у нижнего края, иначе кнопка «вниз»), остальное — полный `renderMessages`. Приватные чаты идут через `reloadPrivateChat`. Deps: `getCurrentChatId/Type`, `getMyUserId`, `getMessages`/`setMessages` (простая замена, без `clearNewerGap`), `setCurrentChatInfo`, `refreshChatList`, `reloadPrivateChat`, `mergePendingUploads`, `reconcilePendingUpload`, `renderMessages`, `appendMessageToView`, `scrollToBottom`, `applyMessageDelete`, `applyMessageEdit`. Тест — `scripts/test-connection.js`.

### `presence.js` → `BF.presence`
Присутствие собеседников: онлайн-статусы (`online_status` → точки `.online-dot[data-online-user]` в списке чатов и «в сети» / «был(а) в сети» в шапке открытого личного чата — не для группы, бота и не-собеседника), подписка на статусы (`collectUserIds()` — собеседники всех личных чатов списка без себя и ботов, подписка меняется только при изменении состава; `subscribeFor(ids)` — добирает новых) и входящий индикатор «печатает…» (`typing` → шапка открытого чата: в личном — «печатает…», в группе — до трёх имён, неизвестного пользователя догружает `getUser` и перерисовывает; таймаут 6 с, повторное событие перезапускает таймер, `action === 2` снимает; статус в шапке не перетирает «печатает…»). `resetTyping()` — при смене чата. Публичное API: `init`, `isOnline`, `getEntry`, `handleStatus`, `resetTyping`, `collectUserIds`, `subscribeFor`. Deps: `getChats`, `getCurrentChatId`, `getCurrentChatInfo`, `getPeerIsBot`, `getMyUserId`, `getCachedUser`, `getUser`. Тест — `scripts/test-presence.js`.
