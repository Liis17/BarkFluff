...
- **Пересланные сообщения**: `MessageAttachmentType.ForwardedMessage` (8) — снапшот оригинала. `ForwardedAuthorName`, `ForwardedOriginalMessageId`, `ForwardedText` хранятся в `MessageAttachments`; вложения оригинала — в `ForwardedMessageAttachments`. Плюс `ForwardedOriginalChatId`/`ForwardedOriginalSenderId`/`ForwardedOriginalSentAt` (для «переслано из ⟨чат⟩ ⟨дата⟩» и перехода к оригиналу) и `ForwardedOrder` — порядок внутри пересылки нескольких сообщений. У снапшотов, созданных до разделения reply/forward, эти колонки NULL
- **Reply и Forward разделены**: у `Message` есть `ReplyToMessageId` (ссылка, не снапшот), у `OutgoingMessage` — `reply_to_message_id` и `forwarded_message_ids` (пачка до 20). Эвристики «оригинал есть в загруженной истории» на клиентах больше нет
- **Reply — ссылка, forward — снапшот**: цитата ответа резолвится при каждой выдаче (`ReplyPreviewResolver`), поэтому правка оригинала видна, а удаление скрывает текст. Пересылка остаётся снапшотом — оригинал может быть в чужом/чужеузловом чате
- **Legacy `forwarded_message_id`** (поле 3) остаётся рабочим для необновлённых клиентов (iOS, macOS, ClientV2.WPF, Linux) и трактуется как **пересылка** — именно так оно и хранилось. Смешивать его с полями 4/5 нельзя → `ConflictingForwardFieldsException` (InvalidArgument). Старые данные не мигрировались: в БД ответ и пересылка неотличимы
...
- **Reply-превью**: `Mapping/ReplyPreviewResolver` собирает `ReplyInfo` для страницы за два батча (один в БД за оригиналами, один в Users за именами) независимо от количества ответов. Удалённый оригинал отдаётся с `is_deleted=true`, пустым текстом и без автора — цитата не должна быть способом прочитать удалённое. Подключён в `ListMessages`, `SendMessage`, `EditMessage`, `PinMessage`, `ListPinnedMessages`; в `ListChats` намеренно нет — там цитата не рисуется
- **Валидация reply**: `Features/Shared/ReplyTargetValidator` — оригинал существует, тот же чат, не удалён. Общий для `SendMessage` и `UpsertChatDraft`: раньше правило жило только в черновике, и выбранный там reply уезжал на сервер уже пересылкой
...
- Reply и forward федерируются: `NewMessagePayload.reply_to_federated_message_id` (межнодовый uuid, локальный `Id` через границу бессмысленен) и `repeated FederatedForward` (снапшот; `original_message_id` не передаётся — он локален для origin). На приёме uuid резолвится через `GetByFederatedIdAsync`; **оригинал не найден → сообщение сохраняется без цитаты, не RETRY** (дыру дотянет catch-up 2.6). Снапшот пересылки валидирует `FederatedForwardImporter` по образцу `FederatedAttachmentImporter` → `FederatedForwardInvalidException` (permanent)
