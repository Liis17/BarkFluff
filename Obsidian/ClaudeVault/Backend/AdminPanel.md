...
На вкладке дополнительного сервера кнопка **«Открыть консоль»** открывает интерактивный SSH-сеанс в модальном окне. Браузер не подключается к SSH напрямую и не получает пароль: AdminPanel сама открывает PTY через SSH.NET и проксирует ввод/вывод через авторизованный WebSocket `/api/remote/servers/{serverId}/console`. Соединение дополнительно проверяет `Origin` и закрывается вместе с модалкой; прокси Nginx для dev/nightly/master передаёт WebSocket Upgrade и держит консольный маршрут до часа.
