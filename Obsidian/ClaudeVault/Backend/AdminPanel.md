...
На вкладке дополнительного сервера кнопка **«Открыть консоль»** открывает интерактивный SSH-сеанс в модальном окне. Браузер не подключается к SSH напрямую и не получает пароль: AdminPanel сначала принимает авторизованный WebSocket `/api/remote/servers/{serverId}/console`, затем открывает PTY через SSH.NET и проксирует ввод/вывод. После успешного открытия shell отправляется служебный ready-сигнал; каждая запись ввода после `ShellStream.WriteAsync` немедленно сбрасывается через `FlushAsync`, чтобы короткие команды не оставались во внутреннем буфере SSH.NET; байты вывода PTY идут в binary WebSocket-фреймах и декодируются в браузере потоковым UTF-8-декодером, чтобы разрезанный символ не закрывал канал. `Pages/v2/assets/terminal.js` поддерживает состояние ANSI/OSC между фреймами, скрывает служебные escape-последовательности, применяет SGR-цвета и базовые операции перерисовки PTY (`CR`, `BS`, CSI очистки/перемещения) безопасными DOM-узлами; при ошибке после WebSocket upgrade панель показывает диагностическое сообщение и закрывает канал. Соединение дополнительно проверяет `Origin` и закрывается вместе с модалкой; прокси Nginx для dev/nightly/master передаёт WebSocket Upgrade и держит консольный маршрут до часа.
...
- `DockerService` — управление Docker-контейнерами. Локальные Compose-операции выполняются через временный helper-контейнер с реальными host-путями проекта и файлами `docker-compose.yml`/`.env`; это сохраняет разрешение относительных bind mounts (например, `./data/postgres`) относительно каталога проекта. Обновление отдельного сервиса использует `--no-deps`, поэтому не пересоздаёт PostgreSQL и другие зависимости.
...
| `dashboard.html` | KPI (включая liveness «X/Y» из `/api/health/overview`), бейдж системного статуса, трафик, метрики сервисов из Seq; карточка последней загрузки [[Backend/Files|Files]] обновляется live раз в 10 секунд |
...
| Last upload refresh | 10 секунд (только раскрытая карточка [[Backend/Files|Files]]; Seq + fallback на LiteDB) |
