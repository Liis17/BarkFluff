...
`TelegramAuthWorker` использует Bot API [getUpdates](https://core.telegram.org/bots/api#getupdates), Redis lease на bot ID с продлением и PostgreSQL offset. Перед разрешением `AuthenticationService` в scope Telegram-обновления worker задаёт пустой `RequestContext`: gRPC-interceptor в фоновой задаче не выполняется, а обработка обновления не должна наследовать данные браузерного устройства или пользователя. Offset сохраняется после обработки обновления; повторный callback безопасен. Ошибка Redis или Telegram не разрешает вход. При сбое polling лог содержит фактический этап операции, stack trace и детали исключения с удалённым токеном; токен не передаётся клиенту и не должен попадать в логи. Конфигурация: `TelegramAuth:Enabled`, `TelegramAuth:BotToken`, `TelegramAuth:NodeName`; доступность почты — `Email:Enabled` (по умолчанию true).
...
Проверки: полный набор Identity, включая старые `Auth`/reset RPC, и полный Users-набор. `AuthenticationFlowTests` дополнительно запускается на PostgreSQL для проверки транзакций, advisory lock и миграций; InMemory-проверки сами по себе эту интеграционную проверку не заменяют. Клиентские потоки описаны в [[Клиенты/Web]], FastAuth — в [[Backend/FastAuth]].
