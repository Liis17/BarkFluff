...
## Подтверждённые контакты и Telegram-регистрация

[[Backend/Identity]] создаёт черновик после подтверждения выбранного канала. `AddDraftUser.registration_id` обеспечивает повторное завершение одной регистрации через UUID пользователя. Совпадение только логина или почты не разрешает изменять чужой черновик. Без email запись `UserContact` не создаётся. `FindByLogin` исключает черновики. Серверный `SetVerifiedEmail` добавляет или меняет контакт только после отдельного подтверждения в Identity; существующая почта активных аккаунтов считается проверенной. См. [[Shared/Proto]].
...
**Uuid пользователя** (Фаза 0 rearch-федерации, [[Backend/Settings#ServiceId=15]]): `Guid`, генерируется в коде (`Guid.NewGuid()`) при создании в обоих путях — `CreateUser` (draft) и `CreateBotUser`. Уникальный индекс `IX_Users_Uuid` (миграция `AddUserUuid`). У колонки есть страховочный `defaultValueSql: "gen_random_uuid()"` на уровне БД — заполнил все существующие строки при бэкфилле и подстрахует прямые INSERT в обход кода. `long Id` остаётся внутренним PK, `Uuid` — будущий межсерверный идентификатор, пока без federation-логики поверх него. Отдаётся в proto `User.uuid` (поле 13, строка). Этап 0.4: `UsersApi.ResolveFederatedUser` (RPC-заглушка, реализация — Фаза 2) + `PrivacySettings.deny_federated_dm` (field 7, default false = федеративные DM разрешены) добавлены в `users_api.proto`, логики за ними пока нет.
...
Для Telegram-регистрации в [[Backend/Identity]] обычный draft может не иметь `UserContact`; пустой email не записывается и не участвует в поиске. `OverrideDraftUser` отклоняет вызов: совпадение логина/email не доказывает владение черновиком и не позволяет переписать подтверждённый аккаунт. Повторная отправка/продолжение регистрации выполняется Identity по секрету исходного запроса.
