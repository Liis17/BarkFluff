...
Legacy-централизованное хранилище конфигурации для всех микросервисов. Порт: **7003**.
**Не использует XAuth** — к нему обращаются сервисы при старте до получения токенов.

Сервис сохранён без удаления и работает параллельно с новым [[Backend/Settings]]. Базовый compose по-прежнему направляет потребителей сюда; переход на Settings выполняется opt-in override и остаётся обратимым.
