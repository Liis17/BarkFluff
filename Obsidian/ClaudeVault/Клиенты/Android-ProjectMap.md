...
- **Поле ввода**: `inputBar` имеет динамическую высоту (`wrap_content`, максимум 4 строки у `messageEditText`), поэтому многострочный текст растит панель вверх; нижний padding `messagesRecyclerView` обновляется по фактической высоте. Кнопка отправки — 52dp в режиме микрофона и 68dp pill в режиме отправки.
