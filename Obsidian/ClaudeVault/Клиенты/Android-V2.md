...
- **Профиль + настройки** (`ui/screens/profile`, `ui/screens/settings`):
  ...
  - `SettingsScreen`: режим темы (Системная/Светлая/Тёмная) + переключатель «Динамические цвета», строка «Конфиденциальность», версия приложения. Хранение оформления — `di/SettingsStore` (собственные SharedPreferences, `StateFlow`), читается в `MainActivity`.
