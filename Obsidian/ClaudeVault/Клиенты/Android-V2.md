...
> ⚠️ **Статус: заброшен, тестовый проект.** Заказчик явно сказал, что V2 в активной разработке не участвует — все доработки Android идут только в V1 (`Android/Barkfluff.Client.Android`, модули `app-v1` + `core`). V2 не трогать без отдельного запроса; описание ниже фиксирует состояние на момент остановки работ, а не актуальную дорожную карту.
...
**Единый стиль экранов (тактика Containment):** общий компонент `ui/components/SettingsSection` + `SettingsRow` группирует связанные строки в скруглённый тональный контейнер (`surfaceContainerHigh`, shape `large` 24dp) с прозрачными `ListItem` внутри — вместо плоских списков с разделителями. Списочные/настроечные экраны (Чаты, Настройки, Конфиденциальность, Безопасность, Профиль) используют `LargeTopAppBar` со схлопыванием (`exitUntilCollapsedScrollBehavior` + `nestedScroll`) для крупной заголовочной иерархии. Экран чата: шапка с `BfAvatar`+именем, кнопка отправки — круглая `FilledIconButton` (главное действие «выпрыгивает» — тактики Color/Size). Эталон отступов/кнопок — экран логина.
...
- **Камера и GIF**: кнопка вложения открывает меню (`ModalBottomSheet`) — фото/видео из галереи (`PickVisualMedia` Image/Video) и съёмка с камеры (`TakePicture`/`CaptureVideo` через `FileProvider`-URI в `cacheDir/media_files`, **без CAMERA-пермишена** — системное приложение камеры). `ChatViewModel.sendVideo` грузит `MESSAGE_ATTACHMENT_VIDEO`. GIF-вложения анимируются: `coil-gif` + `ImageDecoderDecoder` через `ImageLoaderFactory` в `BarkFluffV2Application`; `GifAttachment` резолвит полный файл по `fileId`.
- **Профиль + настройки** (`ui/screens/profile`, `ui/screens/settings`):
  ...
  - `SettingsScreen`: режим темы (Системная/Светлая/Тёмная) + переключатель «Динамические цвета», строки «Конфиденциальность» и «Безопасность», версия приложения. Хранение оформления — `di/SettingsStore` (собственные SharedPreferences, `StateFlow`), читается в `MainActivity`.
  - `PrivacyScreen` (+`PrivacyViewModel`): `getPrivacySettings`/`updatePrivacySettings` — тумблеры «виден на сайте»/«в поиске» + видимость (Все/Друзья/Никто) для аватара/описания/почты/онлайна (DropdownMenu). Каждое изменение сразу шлёт полный `PrivacySettings` на сервер. ⚠️ proto-builder из Java: сеттеры возвращают Builder → в Kotlin вызывать `setX(...)`, не присваивать property.
  - `SecurityScreen` (+`SecurityViewModel`): смена пароля (`changePassword`, ошибка неверного старого → `InvalidOldPasswordException`); 2FA-тумблеры аутентификатора и почты по `listOtpVerification`. Аутентификатор: `getOtpSetup` → диалог с QR (base64→Bitmap) + ручной ключ + ввод кода → `confirmOtpSetup`. Отключение — `disableOtpVerification(type, code)` с вводом текущего кода. Почта — `enableOtpEmail`. Типы — `IdentityApiOuterClass.OtpTypeId.Authenticator/Email`.

**Дальше:** язык (ждёт локализации строк V2), стикеры, папки, pinned-сообщения, регистрация/восстановление, E2E (приватные/секретные чаты), системная интеграция (уведомления/FCM, виджеты, share, deeplink, локализации). E2E-проверка логина/отправки — на устройстве arm64 (libsignal).
