...
- `#connectionBanner` — глобальная доступная плашка проблемы соединения с «Нет сети» / «Переподключаемся…» и ручным «Повторить», видимая также в мобильном списке чатов. После восстановления `js/app/connection.js` (`BF.connection`) ждёт catch-up списка и открытого чата; только успешный проход кратко (3 с) показывает в чате «Синхронизировано». Событие `resync` дебаунсится на 1,2 с в один проход (без открытого чата освежается только список, иначе — хвост чата), `tab_visible` запускает тот же тихий catch-up. Хвост открытого чата сверяет `resyncCurrentChatTail`: дифф по id, правкам, прочтениям и удалениям в диапазоне окна; нет различий — DOM не трогается.

### Список чатов (виртуализация)

`js/app/chat-list.js` (`BF.chatList`) рисует только видимое окно списка: строки видимой области ± 8 (`OVERSCAN`). Высота строки фиксирована — CSS-переменная `--chat-row-height: 75px` на `.chat-list`, JS перечитывает её и нижний padding под плавающую навигацию в `ResizeObserver`.

- Внутри `#chatList` (скролл-контейнер) — `.chat-list-spacer` высотой `N × ROW`; строки `.chat-item` абсолютно позиционированы через `transform: translateY(i × ROW)`. Окно перерисовывается на `scroll` раз в кадр (`requestAnimationFrame`), пагинация по 50 при подходе к низу сохранена.
- `render()` (алиас `renderChatList` в `main.js`) пересчитывает видимые чаты — фильтр папок + **дедуп по id** (offset-пагинация может вернуть чат повторно) — и синхронно перерисовывает окно. Узлы строк переиспользуются по `chat.id`; `innerHTML` строки пишется только если изменилась её HTML-строка (она же сигнатура: черновик, язык, онлайн-точка, превью).
- **Фокус:** строка с фокусом никогда не удаляется и не переносится в DOM (перенос снимает фокус); если её чат исчез (фильтр папки) — фокус переходит соседу.
- **a11y:** `#chatList role="listbox"`, строки `role="option"` с `aria-posinset`/`aria-setsize`, открытый чат — `aria-selected="true"`; `aria-label` строки = название, непрочитанные (`a11y.chat.unread.*`), превью, время. Roving tabindex (`tabindex=0` у одной строки), клавиши ↑/↓/Home/End/PageUp/PageDown, Enter/Space открывают чат (на мобильном — и экран чата через `__mobileShowChat`).
- Смена открытого чата извне списка (палитра, push, deep-link, newchat) прокручивает список к строке, если она не видна.
- Клики, клавиши и `contextmenu` делегированы на `#chatList`. `BF.presence.handleStatus` (`presence.js`) обновляет только отрисованные `.online-dot`; невидимые строки получат актуальный статус при появлении в окне.
...
- Открытие с клавиатуры: Shift+F10 или клавиша ContextMenu на сфокусированной строке чата / сообщении — меню встаёт у элемента и фокус переходит на первый видимый пункт. Следующий за клавишей браузерный `contextmenu` подавляется (500 мс).
- Навигация — общие хелперы `BF.utils.focusMenuItem` / `handleMenuKeydown` (`utils.js`, рядом с focus-trap оверлеев): ↑/↓ с циклом, Home/End, скрытые (`display:none`) и disabled пункты пропускаются; Enter/Space — нативная активация кнопки; Escape и Tab закрывают меню.
- Фокус возвращается, только если при закрытии он был внутри меню: на сообщение-источник / строку чата (по `chatId` через `focusRow`, узел мог быть переиспользован виртуализацией) или на элемент, активный до открытия мышью. Если меню открыто мышью, стрелки переводят фокус в него.
- `BF.utils.openOverlay` здесь не используется — он модальный (focus-trap + `inert`).
- Тест хелперов — `scripts/test-context-menu.js`.

### Лента как ARIA feed

Доступность ленты открытого чата живёт в `feed.js`. Атрибуты ставит только он, одной функцией `decorate(el, msg)` при сборке элемента, а не `messages.js`: `buildMessageElement` рисует и список закреплённых (`pinned.js`), где роли не нужны.

- `#messagesInner` — `role="feed"`, `aria-busy="true"` на время рендера и загрузки страниц (счётчик, поэтому вложенные загрузки не снимают флаг раньше времени). Каждое сообщение — `role="article"`, `aria-label` = «автор, время» (для своих «Вы»; имя резолвится через `getUser` и дописывается позже), `aria-describedby` → `id="feed-msg-<id>"` на облачке. Системные плашки подписаны своим текстом. Разделители дат и «непрочитанных» — обычные `div`.
- **Roving tabindex:** `tabindex="0"` только у одного сообщения (по умолчанию последнее), у остальных атрибута нет — иначе клик мышью по сообщению уводил бы в него фокус. `syncRover()` вызывается после рендера, вставки, подгрузки и обрезки.
- **Клавиши на сообщении:** ↑/↓ — соседнее (на краю окна догружается страница через `loadOlderMessages`/`loadNewerMessages`, затем фокус переходит), PageUp/PageDown — примерно на 0.8 экрана, Home — первое загруженное, End — последнее (при `hasNewerGap` сначала `jumpToLiveTail`). Enter и Shift+F10 / ContextMenu открывают меню сообщения (`message-menu.js`): на macOS нет клавиши ContextMenu.
- **Фокус переживает перестройки:** полный `render()` возвращает фокус тому же сообщению, при удалении — соседу; `trimMessages` при обрезке узла с фокусом переносит его на ближайшее уцелевшее; замена и удаление узлов (pending → серверное сообщение, правка, перегруппировка) идут через `BF.feed.replaceElement`/`removeElement`, которые переносят ровер и фокус; прыжок `scrollToMessage` переносит фокус на цель, если он был в ленте.
- **Live-регион:** статичный `#feedLiveRegion` (`aria-live="polite"`, `.sr-only`) вне ленты. `BF.feed.announceIncoming(msg)` вызывают только `handleNewMessage` в `main.js` (чужое сообщение открытого чата) и `private-chat-ui.js`. История, resync и пагинация не озвучиваются. Текст — `a11y.newMessage` (автор и текст без разметки до 200 символов либо тип вложения); пачка за секунду склеивается в `a11y.newMessages.*`.
- Фокус статьи рисуется вокруг `.msg-bubble`, а не всей строки ленты.
- Тест — `scripts/test-feed.js`.
...
- `chat-list.js::render()` (`BF.chatList`, в `main.js` — алиас `renderChatList`) передаёт актуальный список чатов в рендер вкладок, поэтому realtime-сообщение и событие прочтения сразу обновляют бейдж папки.
...
- Вкладка без открытого чата называется «Мессенджер». Для личного чата — «Чат • Имя Фамилия»; favicon заменяется на круглую, центрированно обрезанную аватарку собеседника. Заголовок с бейджем непрочитанных `(N) …` (`99+` при большем числе), favicon и браузерные уведомления о новых сообщениях (только когда страница видна и открыт другой чат) — `js/app/attention.js` (`BF.attention`).
...
`scrollToMessage(id)` (reply-цитата, панель и список закреплённых в `pinned.js`):

- Цель уже в DOM — плавный `scrollIntoView({block:'center'})` + класс `.highlight` (animation `msgHighlight`, 1.5 с).
- Цель не загружена — `loadMessagesPage(chatId, id, 30, 30)` (работает и в приватных/E2E) и окно **заменяется целиком** (паттерн `jumpToLiveTail`), а не мержится: merge оставлял дыру в истории без флага `hasNewerGap`. Флаги окна выставляются по числу сообщений старее/новее цели (не по размеру ответа — `api.js` подменяет `offsetBefore=0` на 30): `< 30 старее → noMoreOlder`, `≥ 30 новее → hasNewerGap`.
- После рендера цель центрируется и подсвечивается `settleHighlight(id)`; пере-центрование после догрузки картинок — общий хелпер `resettleAfterImages(anchor)` (выделен из `settleScroll`).
- Цель не найдена (удалена) — тост `chat.messageNotFound`. Guard `isJumpingToMessage` исключает гонки с `jumpToLiveTail`, `loadNewerMessages` и подгрузкой старых на скролле.

### Разделители «Непрочитанные» / «Новые сообщения»

- **При открытии чата** — pill перед `firstUnreadMessageId` из chat info (ключ `chat.unreadMessages`).
- **После resync-догрузки** (`BF.connection.resyncCurrentChatTail`) — pill «Новые сообщения» (ключ `chat.newMessages`) перед первым пропущенным сообщением, если пользователь **не** был у нижнего края (был у низа — догруженное сразу помечается прочитанным, разделитель не нужен). Ставится и в ветке точечной догрузки (`appendMessageToView(m, separatorKey)` — вставка между разделителем даты и сообщением), и в ветке полного ререндера (якорь `resyncSeparatorId` читает сам `renderMessages`, приоритетнее `firstUnreadMessageId`). Если окно было в середине истории (`hasNewerGap`) и resync-новости проглотил guard, pill появляется при пересечении якоря в `loadNewerMessages` (достраника вперёд).
- Жизненный цикл `resyncSeparatorId`: сбрасывается при смене чата, `jumpToLiveTail`, далёком прыжке `scrollToMessage`, прокрутке ниже pill'а (проверка в scroll-handler по `data-sep-key="chat.newMessages"`) и `removeOrphanSeparators` (осиротевший разделитель — удаляемое сообщение было единственным после него). Приватные чаты якорь игнорируют (`currentChatType === 1` guard в `renderMessages`): их resync идёт через `reloadCurrentPrivateChat` минуя `resyncCurrentChatTail`.
- `removeOrphanSeparators()` чистит и `.msg-unread-separator`, а не только даты.
...
## Функции
Согласие с документами (гейт на все три входа), логин + 2FA, регистрация, fast-auth (QR), список чатов и сообщения (отправка/редактирование/удаление/закреп/прочитано, вложения, пересылка, прыжок к сообщению по reply/закрепу с окном ±30 и подсветкой), папки, настройки (профиль/сессии/2FA/хранилище/**язык**), персонализация, **звонки (аудио/видео, 1-на-1 и группы)**, локализация на 5 языков, разделители «Непрочитанные»/«Новые сообщения» (после resync-догрузки) в ленте.
...
### Регистрация, вход и факторы подтверждения

- `register.js` собирает логин и пароль, затем позволяет выбрать подтверждение по почте либо через Telegram. Почта не требуется при Telegram-регистрации. Для Telegram предлагается режим «пароль + второй фактор»; сессию браузер сохраняет только после завершения подтверждения Identity. Продолжение черновика идёт по исходному challenge secret, а проверка совпадения логина/email не даёт переписать чужую регистрацию.
- `login-page.js` загружает доступность способов текущей ноды через `GetAuthCapabilities`. Доступный Telegram-вход и режимы с Email OTP скрываются/блокируются, если соответствующий способ на ноде выключен. Регистрация не продолжает шаг подтверждения, пока нода не подтвердила доступность Email или Telegram. Режимы обычного входа — пароль, Telegram approval без пароля, пароль плюс выбранный Email/TOTP/Telegram фактор; второй фактор можно сменить в форме, если аккаунту разрешён более одного доступного способа. Режим кнопочного входа должен совпадать с настройкой аккаунта; при неверном парольном режиме после успешной проверки пароля веб показывает явное сообщение через `AuthChallengeResponse.error_code`, а для безпарольного Telegram-входа сохраняет нейтральное ожидание, чтобы не раскрывать аккаунты по логину. При таком ожидании веб предлагает проверить раздел «Безопасность».
- `js/app/auth-challenge.js` (`BF.confirmations`) строит запросы Identity, связывает попытку с выбранной нодой и ограничивает сетевые вызовы политиками `READ`/`MUTATION`. Статус ожидающей операции опрашивается раз в 2 секунды. Закрытие окна, смена ноды и `pagehide` отменяют challenge; ответ от предыдущей ноды или поколения попытки не применяется к форме и сессии.
- `js/app/auth-confirmation.js` (`BF.authUI`) показывает ожидание кнопки «Подтвердить»/«Отклонить» в боте, переход в Telegram, ввод OTP/резервного кода, повторную отправку, отказ, истечение срока и подтверждение настроек. Для кнопочного входа ожидание объясняет, что выбранный режим должен совпадать с настройкой аккаунта; повторная отправка называется запросом подтверждения, не кодом. Нативные диалоги центрируются явным `margin: auto`, чтобы общий CSS-сброс страницы не сдвигал их к левому верхнему углу; отправка OTP-кода кнопкой и клавишей Enter ведёт к одному обработчику. Recovery меняет пароль ограниченным proof и возвращает пользователя на обычный вход. Резервный набор показывается отдельным диалогом при выпуске.
- Экран «Безопасность» настроек управляет режимом входа, предпочитаемым фактором, Telegram-связью и его применением для входа/2FA/FastAuth, подтверждённой почтой, TOTP, резервными кодами и отключением факторов. Изменения проходят через повторную аутентификацию; отключение Telegram и его отвязка представлены отдельно. Доступность Email и Telegram зависит от конфигурации выбранной ноды. Подробные серверные ограничения и контракты см. [[Backend/Identity]] и [[Shared/Proto]].
- Раздел «Уведомления» отдельно задаёт обязательный канал оповещений об успешных входах и неверных паролях. Доступны только подтверждённая почта и привязанный Telegram, настроенный на текущей ноде; если оба канала подключены, пользователь выбирает один, а если доступен только один — он используется без возможности отключить оповещения. Серверные правила и доставка описаны в [[Backend/Identity]] и [[Backend/Notification]].
- `fast-auth.js` сохраняет текущий QR attempt и общий дедлайн при переподключении потока. После сканирования и одобрения QR отображает ожидание Telegram, если оно включено в политике аккаунта; старые события не могут завершить новую попытку. Серверная проверка выпуска сессии описана в [[Backend/FastAuth]].
...
Видимая вкладка оставляет уведомление текущему realtime-клиенту; скрытая или закрытая — service worker через FCM. Click фокусирует существующий клиент либо открывает `/messenger?chat=...`; `js/app/deep-link.js` (`BF.deepLink`) открывает чат после загрузки списка. Worker группирует уведомления tag'ом чата/звонка и закрывает их по dismiss-событию. Секретные сообщения, E2E-инвайты, звонки и админ-рассылки не раскрывают содержимое: Web получает только технические ID, имя/аватар отправителя там, где они нужны.
