...
- `js/app/calls-ui.js` (`BF.callsUI`) — UI: ринг-оверлей входящего, полноэкранный экран активного звонка (сетка плиток + контролы микро/камера/демонстрация/сброс), рингтон (WebAudio), медиа через `window.LivekitClient` (Room: connect, публикация треков, привязка `TrackSubscribed`/`ParticipantConnected` к плиткам). Имя/аватар участника резолвится через `BF.api.getUser` (identity LiveKit-токена = userId). Кнопки — inline-SVG (не эмодзи); выключённый микрофон/камера подсвечиваются красным с перечёркнутой иконкой; говорящий участник подсвечивается (`ActiveSpeakersChanged` → класс `.speaking`); демонстрация экрана (`Track.Source.ScreenShare`) рендерится крупным «stage» сверху (`object-fit:contain`), остальные плитки — узкой полосой снизу; пока собеседник не подключился — компактный экран ожидания (аватар + pulse-анимация + локальное self-превью в углу). **Качество** (поповер-ползунки): «Голос · для всех» → `BF.calls.setAudioQuality` (общее, через сервер, применяется по событию `audio_quality_changed` переопубликацией микрофона с `audioPreset`); «Видео · ваш стрим» (видно только при включённой камере) → локально меняет разрешение+битрейт своей камеры (republish, на сервер не ходит).
