...
`SettingsPage` — master-detail оболочка с четырнадцатью разделами. Разделы «Безопасность», «Аккаунт» и «Приватность» уже подключены к [[Клиенты/Windows-WebApiCore]]; остальные пока содержат каркас интерфейса и явно помечают недоступные возможности. Приватность сохраняется оптимистично и при ошибке повторно загружает серверное состояние.
