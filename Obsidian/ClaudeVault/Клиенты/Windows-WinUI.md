...
Workflow `.github/workflows/build-client-winui.yml` запускается для изменений WinUI-клиента в ветках `master`, `dev` и `nightly`, а также вручную. Для одной ветки включена последовательная очередь сборок, чтобы nightly-публикации не получили одинаковую версию. Workflow восстанавливает PFX self-signed сертификат из секрета `WINUI_MSIX_CERTIFICATE_B64`, импортирует его в хранилище сертификатов runner-а и подписывает им MSIX. Секрет должен содержать полный Base64 PFX; workflow также удаляет пробелы, кавычки, Markdown-обрамление, стандартные метки `Base64:`/`PFX:` и маркеры `BASE64_BEGIN`/`BASE64_END`, если они попали туда при копировании. PEM-сертификат для этого секрета не подходит. Пароль PFX хранится в `WINUI_MSIX_CERTIFICATE_PASSWORD`; сертификат должен соответствовать `Publisher` в `Package.appxmanifest`.

Перед публикацией workflow временно выбирает вариант MSIX и восстанавливает исходные файлы после завершения: `master` → канал `release`, каталог иконок `Assets\master`, identity `7895OrbitinSpace.Barkfluff`, имя `Barkfluff`; `dev` → `dev`, `Assets\dev`, `7895OrbitinSpace.Barkfluff.Dev`, `Barkfluff.dev`; `nightly` → `nightly`, `Assets\nightly`, `7895OrbitinSpace.Barkfluff.Nightly`, `Barkfluff.nightly`. Publisher у всех вариантов общий, поэтому стабильный `master` продолжает обновляться, а три identity устанавливаются рядом.

Источник версии для всех вариантов — `/get/barkfluffwinui/nightly/version`, затем legacy fallback `/get/barkfluffwinui/beta/version`, затем версия исходного manifest. Только `nightly` увеличивает patch на единицу; `master` и `dev` используют текущую nightly-версию без изменения. В ClientStorage и имени архива используется `1.2.3`, в MSIX и assembly — `1.2.3.0`; автоматическое увеличение revision отключено. WinUI workflow и встроенный updater используют системное хранилище доверенных сертификатов Windows для публичного `storage.barkfluff.com`; Cloudflare Origin CA в пакет не вкладывается.

Папка установщика архивируется в файл `BarkFluffWinUI_{ветка}_{версия}.zip` и загружается в ClientStorage: `master` — через `/set/barkfluffwinui`, `dev` и `nightly` — через `/set/barkfluffwinui/{channel}`. Для загрузки используются секреты `CLIENT_STORAGE_URL` и `CLIENT_STORAGE_TOKEN`; для подписи нужен секрет PFX-сертификата WinUI. После успешной загрузки workflow отправляет success-уведомление в Telegram с кнопкой «Скачать последнюю версию» на `https://storage.barkfluff.com/get/barkfluffwinui/{channel}`; при ошибке — кнопку «Открыть GitHub Action». На Windows для Telegram используется Schannel с `--ssl-revoke-best-effort`, а сбой уведомления не меняет итог сборки.
...
`SettingsPage` — master-detail оболочка с тринадцатью разделами. Раздел «Виджеты» удалён из навигации Windows-клиента. Разделы «Безопасность», «Аккаунт», «Приватность», «Устройства» и «Уведомления» уже подключены к [[Клиенты/Windows-WebApiCore]]; «Данные и кеш» отображает серверную квоту и разбивку по типам файлов. Локальные кеши по-прежнему недоступны и явно помечены как Android-only. Приватность сохраняется оптимистично и при ошибке повторно загружает серверное состояние.

Загрузка «Устройств» считает неполный успешный ответ (без текущего устройства или списка сессий) ошибкой загрузки и показывает `Error_SettingsLoadFailed`, не обращаясь к отсутствующим данным. Блок подключения устройства по QR-коду из настроек удалён; QR-авторизация на экране входа остаётся. Другие устройства показываются во встроенном прокручиваемом списке высотой максимум в пять строк (`MaxHeight=280` при стандартной строке 56 px). Кнопка «Завершить все другие сессии» видна только при наличии таких устройств и после подтверждения последовательно вызывает существующий `RemoveSessionAsync`; текущая сессия не затрагивается, успешно завершённые устройства удаляются из списка, а неуспешные остаются для повторной попытки.

«Общие» также содержит включённое по умолчанию запоминание размера и положения окна. При первом запуске и после сброса используется размер `1000×800`, окно центрируется на рабочей области главного экрана; сохранённые размеры и координаты отображаются в настройках, записываются в JSON-группу `settings.window` и восстанавливаются при следующем запуске. Изменения размера или положения откладывают запись на короткое время, а перед настоящим выходом актуальные границы принудительно сохраняются. Сброс одновременно очищает сохранённые координаты, возвращает размер к `1000×800` и центрирует уже открытое окно.
...
«Обновление» определяет канал по identity установленного MSIX (`7895OrbitinSpace.Barkfluff` → `release`, `.Dev` → `dev`, `.Nightly` → `nightly`) и обращается только к своему endpoint `https://storage.barkfluff.com/get/barkfluffwinui/{channel}/version`. Чужой канал нельзя выбрать в UI. По кнопке «Обновить» клиент скачивает ZIP своего канала, распаковывает его во временный каталог, выбирает MSIX с тем же identity и более новой версией и открывает его через системный App Installer. Сравнение нормализует `1.2.3` и `1.2.3.0`; по умолчанию соединение проверяется системным хранилищем доверенных сертификатов Windows, а упакованный CA-бандл используется только если он присутствует в пакете. «О приложении» по флагу тестирования показывает адреса микросервисов и проверяет Beacon вызовом `GetServerInfo`. Все четыре флага «Тестирование» сохраняются в JSON-группу `settings.testing`; пока их читает только флаг отображения адресов.
