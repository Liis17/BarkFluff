...
Ветка `master` или `release` публикуется в канал `release` через `/set/barkfluffwinui`; ветки `beta`, `dev` и `nightly` — через `/set/barkfluffwinui/{channel}`. Папка установщика архивируется в файл `BarkFluffWinUI_{ветка}_{версия}.zip` и загружается в ClientStorage. Для загрузки используются `CLIENT_STORAGE_URL`, `CLIENT_STORAGE_TOKEN` и `CLOUDFLARE_ORIGIN_CA_BUNDLE_B64`.
