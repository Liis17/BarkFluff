...
«Общие» также содержит включённое по умолчанию запоминание размера окна. При первом запуске и после сброса используется размер `1000×800`; сохранённые размеры отображаются в настройках, записываются в JSON-группу `settings.window` и восстанавливаются при следующем запуске. Сброс одновременно возвращает сохранённые значения и уже открытое окно к `1000×800`.
