...
`MessengerViewModel.MessageActions.cs` (542 строки) для этапа 4 уже написана — остаётся только разметка: `ContextFlyout`/`MenuFlyout`, подтверждения через `IDialogService`, плашка закрепа, цитата, режимы правки и ответа в композере.
