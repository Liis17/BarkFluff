...
- Для edge-to-edge регистрация вручную применяет system-bar/display-cutout/IME-инсеты к header, контенту и нижней CTA-панели; шаги на `NestedScrollView` получают нижний запас для прокрутки, а фокусируемое поле автоматически доводится до видимой области над клавиатурой.
