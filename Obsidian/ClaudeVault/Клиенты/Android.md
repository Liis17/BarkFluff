...
- Пока экран выбора ноды видим, каждая прикреплённая карточка делает probe сразу и повторяет его не чаще одного раза в 2 секунды; значение «Отклик» обновляется после каждого завершённого round-trip. Опрос отменяется при уходе строки с экрана, остановке Activity и начале подключения; кеш успешного отклика не используется.
