...
Двенадцать Activity, открываемых из `ProfileFragment` (аккаунт, безопасность, приватность, устройства, персонализация, папки, уведомления, язык, данные и кеш, обновление, о приложении и тестирование), используют локальную тему `Theme.BarkfluffClientAndroid.Settings2A`. Она не меняет остальные экраны: в светлой и тёмной темах задаёт тёплый container-фон, светлые surface-плашки, primary/tonal/error-роли, soft-card и split-card формы `28/6dp` с зазором 3dp.
