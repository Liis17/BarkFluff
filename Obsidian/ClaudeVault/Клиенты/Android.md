...
- Мини-лейблы над полями (`TextAppearance.Barkfluff.Register.FieldLabel`), поля FilledBox с `colorSurfaceContainerHighest`, высота 56dp и скругление 16dp. Текст и курсор центрируются по вертикали, для полей включены видимый 2dp focus-indicator, `labelFor`, autofill hints и последовательность IME `Next` → `Done`.
