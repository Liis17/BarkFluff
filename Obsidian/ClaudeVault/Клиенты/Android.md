...
- `CallActivity` рисует **адаптивный grid** плиток (`CallTileView` per-участник, renderer инициализируется один раз и привязывается к треку из модели): 1-на-1 — крупная плитка + self-PiP, группа — сетка до 2 колонок, активная демонстрация экрана — приоритетная плитка сверху + полоса камер снизу. Tap по плитке — focus/возврат к сетке. Плитка показывает имя, индикатор mute и обводку говорящего.
- Контролы: mic, камера, **переворот камеры** (виден при включённой камере), демонстрация экрана, overflow-лист (`Маршрут звука` / `Качество голоса` / `Качество видео собеседника`), завершить. Состояния кнопок (selected/disabled) и foreground service синхронизированы с моделью участников; foreground обновляется только при смене camera/screen.
