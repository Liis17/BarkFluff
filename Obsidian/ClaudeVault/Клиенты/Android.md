...
- Состояния каталога разделены на loading/content/empty/error: пустой или недоступный Navigator больше не подменяется тестовыми нодами; empty/error показывают локальное сообщение и кнопку «Повторить», ручное подключение остаётся доступным.
