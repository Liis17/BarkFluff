...
- **Грядка ввода**: `TextInputLayout` + четыре отдельные круглые кнопки заменены на `inputBar` — pill с минимумом 52dp (`bg_chat_input_bar`) со скрепкой, `EditText` (`messageEditText`, уже не `TextInputEditText`) и стикерами внутри. Поле допускает до 4 строк, `inputBar` растёт по высоте вверх, а нижний padding `messagesRecyclerView` пересчитывается по фактической высоте. `inputRowBarrier` теперь ссылается на `inputBar` и `sendButton`.
