...
- Для edge-to-edge регистрация вручную применяет system-bar/display-cutout/IME-инсеты к header и контенту, а прозрачная CTA-панель позиционируется overlay-слоем над клавиатурой; шаги на `NestedScrollView` получают нижний запас под кнопку, а фокусируемое поле автоматически доводится до видимой области.
...
- `GrpcClientRegistry` учитывает конфигурацию interceptor’ов при переиспользовании слота: если Identity-клиент был создан Login/SelectServer без device metadata, регистрация заменяет его каналом с `DeviceInfoInterceptor` и `x-device-name`.
