...
- Заголовок чата и заглушка «выберите чат» в `MessengerView` привязаны к `SelectedChat` через `StringToVisibilityConverter`, который проверяет `value is string`. Так как `SelectedChat` — `ChatItemViewModel`, оба элемента всегда скрыты.
