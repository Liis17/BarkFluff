...
- `MessengerService` инкапсулирует [[Клиенты/Windows-WebApiCore]] для `MessengerViewModel`: чаты, историю, отправку, файлы и read API. История использует виртуализированный `ui:ListView`: если непрочитанных нет, View прокручивает её вниз; иначе загружает страницу вокруг `FirstUnreadMessageId` и центрирует его. `MessageListBehavior` сообщает VM о сообщениях, видимых минимум на 50%; обычные чаты отмечаются через `MarkMessageAsRead`, private — через `MarkPrivateMessagesAsRead`.
