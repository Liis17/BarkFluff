...
- `Chat.picture` и `UserData.ProfilePicturePreviewUrl` сервер отдаёт готовыми ссылками (`{base}/web/download/{guid}`), а не идентификаторами: они идут прямо в `AvatarUrl`. Разрешать их через `ResolveFileUrlAsync` нельзя — [[Backend/Files]] парсит `file_ids` как `Guid` и отвечает ошибкой. Через `ResolveFileUrlAsync` идут только настоящие идентификаторы: `attachment.FileId` и `attachment.PreviewFileId`.
